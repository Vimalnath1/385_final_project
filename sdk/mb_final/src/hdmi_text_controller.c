...
void setColorPalette (uint8_t color, uint8_t red, uint8_t green, uint8_t blue)
{
 ...
 uint32_t rgb = (red << 8) | (green << 4) | (blue);
 uint32_t index = color / 2;

    if (color % 2 == 0) //check if even
    {
        // even color, so lower 12 bits
    	hdmi_ctrl->PALETTE[index] &= 0xFFFFF000; // clear lower 12 bits
    	hdmi_ctrl->PALETTE[index] |= rgb;
    }
    else
    {
        // odd color, so upper 12 bits (bits 16 to 27)
    	hdmi_ctrl->PALETTE[index] &= 0xF000FFFF; // clear upper color field
    	hdmi_ctrl->PALETTE[index] |= (rgb << 16);
    }
